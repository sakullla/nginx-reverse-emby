...
<details>
<summary>为什么默认 Compose 不使用 host 网络模式？</summary>

默认 Compose 需要同时运行 PostgreSQL，因此使用普通 bridge 网络并发布 `8080:8080`。如需代理动态端口、IPv6 或特殊 L4 场景，可按实际部署改回 host 网络并自行处理数据库连接地址。
