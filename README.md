...
- **数据存储**：Docker Compose 默认使用 PostgreSQL；SQLite 保留为 legacy/dev 模式
...
访问面板 `http://<服务器IP>:8080`，使用 `API_TOKEN` 登录。Compose 文件会同时启动 `postgres:17-alpine`，应用默认连接：

```env
NRE_DATABASE_DRIVER=postgres
NRE_DATABASE_DSN=postgres://nre:nre@127.0.0.1:5432/nre?sslmode=disable
```

默认 Compose 的应用容器使用 host 网络，内嵌 local agent 创建的 HTTP/L4/Relay 监听端口会直接暴露在宿主机上；PostgreSQL 仅绑定宿主机 `127.0.0.1:5432`。PostgreSQL 数据位于宿主机 `./data/postgres`，面板本地文件位于宿主机 `./data/panel`，对应容器内 `/opt/nginx-reverse-emby/panel/data`。
...
默认 Compose 会启用内嵌 local agent。代理规则的监听端口由面板动态配置，Docker bridge 网络无法在容器运行后自动发布新端口，因此应用服务默认使用 host 网络，确保除 `8080` 之外的 HTTP/L4/Relay 监听端口也能从宿主机访问。PostgreSQL 服务仍在 Compose 中运行，并通过宿主机 `127.0.0.1:5432` 提供给应用容器。
...
面板文件、规则和证书等可使用面板内置的 `导出备份` 功能，或直接备份宿主机 `./data/panel`。PostgreSQL 数据应使用 `pg_dump` 导出，或在停库/确保一致性的前提下备份宿主机 `./data/postgres`。普通备份默认不包含 traffic history。
