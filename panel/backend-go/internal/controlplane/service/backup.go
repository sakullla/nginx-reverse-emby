...
func (s *backupService) importRelayListeners(ctx context.Context, existing []storage.RelayListenerRow, incoming []BackupRelayListener, agentIDMap map[string]string, certIDMap map[int]int, result *BackupImportResult, modifiedAgents map[string]bool) (map[int]int, error) {
	...
	for _, item := range incoming {
		...
		normalized, err := normalizeRelayListenerInput(input, RelayListener{}, 0, relayNormalizeOptions{})
		if err != nil {
		...
		if _, exists := usedIDs[assignedID]; exists {
			assignedID = maxID + 1
		}
		if assignedID > maxID {
			maxID = assignedID
...
func (s *backupService) importHTTPRules(ctx context.Context, incoming []BackupHTTPRule, agentIDMap map[string]string, listenerIDMap map[int]int, result *BackupImportResult, modifiedAgents map[string]bool) error {
	...
	for _, row := range existingRules {
		...
		if row.ID > maxIDs[row.AgentID] {
			maxIDs[row.AgentID] = row.ID
		}
		if usedIDs[row.AgentID] == nil {
	...
	for _, item := range incoming {
		...
		assignedID := item.ID
		if assignedID <= 0 {
			assignedID = maxIDs[resolvedAgentID] + 1
		}
		if usedIDs[resolvedAgentID] == nil {
			usedIDs[resolvedAgentID] = map[int]struct{}{}
		}
		if _, exists := usedIDs[resolvedAgentID][assignedID]; exists {
			assignedID = maxIDs[resolvedAgentID] + 1
		}
		if assignedID > maxIDs[resolvedAgentID] {
			maxIDs[resolvedAgentID] = assignedID
		}
