...
func TestBackupServiceImportSkipsWireGuardL4TunnelListenConflictsWhenUDPDefaultsTransparent(t *testing.T) {
	...
	if preview.Summary.Imported.L4Rules != 1 || preview.Summary.SkippedConflict.L4Rules != 1 || preview.Summary.SkippedInvalid.L4Rules != 0 {
	...
	if result.Summary.Imported.L4Rules != 1 || result.Summary.SkippedConflict.L4Rules != 1 || result.Summary.SkippedInvalid.L4Rules != 0 {
		t.Fatalf("import summary = %+v", result.Summary)
	}
	l4Rules, err := targetStore.ListL4Rules(ctx, "edge-wg")
	if err != nil {
	...
	got := l4RuleFromRow(l4Rules[0])
	if got.ListenMode != "wireguard" || got.Protocol != "udp" || got.WireGuardInboundMode != "transparent" {
		t.Fatalf("restored rule = %+v, want udp wireguard transparent", got)
	}
}
...
func TestBackupServicePreviewAndImportDefaultWireGuardL4ListenHostBeforeConflictsWhenUDPDefaultsTransparent(t *testing.T) {
	...
	if preview.Summary.Imported.L4Rules != 1 || preview.Summary.SkippedConflict.L4Rules != 2 || preview.Summary.SkippedInvalid.L4Rules != 0 {
		t.Fatalf("preview summary = %+v", preview.Summary)
	}
...
func TestBackupServiceImportSkipsWireGuardTransparentAndAddressL4RuntimeListenConflicts(t *testing.T) {
	...
	bundle := BackupBundle{
		...
		WireGuardProfiles: []BackupWireGuardProfile{{
			...
			Mode:       "generic_wireguard",
			PrivateKey: testWireGuardPrivateKey,
			Addresses:  []string{"10.101.0.2/32"},
			Peers: []WireGuardPeer{{
				Name:       "peer-a",
				PublicKey:  testWireGuardPublicKey,
				AllowedIPs: []string{"10.101.0.1/32"},
			}},
			Enabled: true,
		}},
		L4Rules: []BackupL4Rule{
			{
				ID:                   91,
				AgentID:              "edge-wg",
				Name:                 "wg-l4-transparent",
				Protocol:             "tcp",
				ListenHost:           "10.101.0.20",
				ListenPort:           8443,
				Backends:             []L4Backend{{Host: "10.101.0.20", Port: 8443}},
				LoadBalancing:        L4LoadBalancing{Strategy: "round_robin"},
				ListenMode:           "wireguard",
				WireGuardProfileID:   &profileID,
				WireGuardInboundMode: "transparent",
			...
			},
			{
				...
				ListenHost:           "0.0.0.0",
				ListenPort:           8443,
				Backends:             []L4Backend{{Host: "10.101.0.21", Port: 8443}},
				LoadBalancing:        L4LoadBalancing{Strategy: "round_robin"},
				ListenMode:           "wireguard",
				WireGuardProfileID:   &profileID,
				WireGuardInboundMode: "address",
				WireGuardListenHost:  "10.101.0.1",
				Enabled:              true,
			},
		},
	}
...
func TestBackupServiceImportAllowsWireGuardL4TunnelListenReuseAcrossProfilesWhenUDPDefaultsTransparent(t *testing.T) {
	...
	if preview.Summary.Imported.L4Rules != 2 || preview.Summary.SkippedConflict.L4Rules != 0 || preview.Summary.SkippedInvalid.L4Rules != 0 {
		t.Fatalf("preview summary = %+v", preview.Summary)
	}
	...
	if result.Summary.Imported.L4Rules != 2 || result.Summary.SkippedConflict.L4Rules != 0 || result.Summary.SkippedInvalid.L4Rules != 0 {
		t.Fatalf("import summary = %+v", result.Summary)
	}
	l4Rules, err := targetStore.ListL4Rules(ctx, "edge-wg")
	if err != nil {
	...
	if len(l4Rules) != 2 {
		t.Fatalf("imported l4 rules = %+v, want exactly two", l4Rules)
	}
	for _, row := range l4Rules {
		got := l4RuleFromRow(row)
		if got.ListenMode != "wireguard" || got.Protocol != "udp" || got.WireGuardInboundMode != "transparent" {
			t.Fatalf("restored rule = %+v, want udp wireguard transparent", got)
		}
	}
...
func TestBackupServicePreviewAndImportSkipWireGuardRulesWhenAgentLacksCapability(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if result.Summary.SkippedInvalid.WireGuardProfiles != 1 || result.Summary.SkippedInvalid.WireGuardClients != 1 || result.Summary.SkippedInvalid.RelayListeners != 1 || result.Summary.SkippedInvalid.HTTPRules != 1 || result.Summary.SkippedInvalid.L4Rules != 1 {
				t.Fatalf("%s summary skipped invalid = %+v, want wg_profiles=1 wg_clients=1 relay=1 http=1 l4=1", tc.name, result.Summary.SkippedInvalid)
			}
	...
	} else if len(rules) != 1 || rules[0].ListenMode != "proxy" {
		t.Fatalf("imported l4 rules = %+v, want proxy entry only", rules)
	}
}
