...
func TestPluginHostRealMutualTLSRestartUsesFreshIdentity(t *testing.T) {
	...
	for attemptNumber := 0; attemptNumber < 2; attemptNumber++ {
		...
		plan, err := actionClient.PlanAction(t.Context(), pluginsdk.RPCActionRequest{Generation: "g1", ActionID: "rotate", OperationID: "operation-plan", ResourceHandle: "handle-plan"})
		...
		action, err := actionClient.InvokeAction(t.Context(), pluginsdk.RPCActionRequest{Generation: "g1", ActionID: "rotate", OperationID: "operation-1", ResourceHandle: "handle-plan", ResourceResults: []pluginsdk.RPCResourceResult{{RequestID: "resource-call-1", Value: []byte(`{"available":true}`)}}})
		...
		for _, test := range []struct {
			...
			response, invokeErr := actionClient.InvokeAction(t.Context(), pluginsdk.RPCActionRequest{Generation: "g1", ActionID: test.actionID, TargetKind: "relay", TargetID: "relay-1", OperationID: test.operation})
