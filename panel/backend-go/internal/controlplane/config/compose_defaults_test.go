...
func TestComposeDefaultTimezoneUsesRuntimeTzdata(t *testing.T) {
	...
	for _, forbidden := range []string{"bubblewrap", "bwrap", "prlimit"} {
		if strings.Contains(controlPlaneBody, forbidden) {
			t.Fatalf("control-plane-runtime image must not install external plugin launcher helper %q", forbidden)
		}
	}
...
func TestComposeDoesNotRequirePrivilegedPluginLauncherSettings(t *testing.T) {
	composePath := filepath.Join("..", "..", "..", "..", "..", "docker-compose.yaml")
	body, err := os.ReadFile(composePath)
	if err != nil {
		t.Fatalf("ReadFile(%s) error = %v", composePath, err)
	...
	compose := string(body)
	for _, forbidden := range []string{"cgroup: host", "seccomp=unconfined", "/sys/fs/cgroup:/sys/fs/cgroup:rw", "bubblewrap", "prlimit"} {
		if strings.Contains(compose, forbidden) {
			t.Fatalf("docker-compose.yaml must not require privileged/external launcher setting %q", forbidden)
		}
	}
}
...
func TestBeginnerComposeDeployScriptExists(t *testing.T) {
	...
	for _, want := range []string{
		...
		"delete_env_value \"NRE_PANEL_PUBLIC_PATH\"",
		"--force-recreate",
		"wait_public_panel_ready",
		"frontend_url conflicts with existing rule",
		"HTTPS 面板已可访问",
