...
describe('runtime canonical rule payloads', () => {
  ...
  it('L4 form keeps relay layers and egress profile as independent selectors', async () => {
    const l4Form = await import('../components/L4RuleForm.vue?raw')

    expect(l4Form.default).toContain('name="egress-profile"')
    expect(l4Form.default).toContain('relay_layers: Array.isArray(form.value.relay_layers)')
    ...
    expect(l4Form.default).toContain('payload.egress_profile_id = selectedEgressProfileID.value')
    expect(l4Form.default).not.toContain('proxy_egress_mode')
    expect(l4Form.default).not.toContain('wireguard_egress_uri')
    expect(l4Form.default).toContain('if (requiresBackends.value && validBackends.length === 0)')
  })

  it('L4 form does not expose legacy WireGuard egress URI controls', async () => {
    const l4Form = await import('../components/L4RuleForm.vue?raw')

    expect(l4Form.default).not.toContain('wireguard_egress_source')
    expect(l4Form.default).not.toContain('wireguard_egress_uri')
    expect(l4Form.default).not.toContain('WireGuard 连接 URI')
    expect(l4Form.default).not.toContain('<option value="uri">WireGuard URI</option>')
  ...
  it('L4 form restricts WireGuard selection to enabled numeric profiles', async () => {
    const l4Form = await import('../components/L4RuleForm.vue?raw')

    expect(l4Form.default).toContain('enabledWireGuardProfiles')
    expect(l4Form.default).toContain('selectedWireGuardProfileID')
    expect(l4Form.default).toContain('Number.isInteger(id) && id > 0')
    expect(l4Form.default).not.toContain('payload.wireguard_profile_id = String')
  })

  it('Relay listener form uses automatic WireGuard profiles without exposing profile selection', async () => {
    const form = await import('../components/RelayListenerForm.vue?raw')
    const source = form.default
    expect(source).toContain('高级设置')
    expect(source).not.toContain("v-model.number='form.wireguard_profile_id'")
    expect(source).not.toContain('WireGuard 配置')
    expect(source).not.toContain('selectedWireGuardProfileID')
    expect(source).not.toContain('payload.wireguard_profile_id')
    expect(source).toContain('Relay TLS 证书 / Pin 校验仍会在隧道内生效')
  })
  ...
  it('Egress profile UI is wired into settings and rule forms without a standalone route', async () => {
    const router = await import('../router/index.js?raw')
    const sidebar = await import('../components/layout/Sidebar.vue?raw')
    const bottomNav = await import('../components/layout/BottomNav.vue?raw')
    const settingsPage = await import('../pages/SettingsPage.vue?raw')
    const settingsNav = await import('../components/settings/SettingsNav.vue?raw')
    const page = await import('../pages/EgressProfilesPage.vue?raw')
    const form = await import('../components/egress/EgressProfileForm.vue?raw')
    const httpForm = await import('../components/RuleForm.vue?raw')
    const legacyHTTPForm = await import('../components/rules/RuleForm.vue?raw')
    const l4Form = await import('../components/L4RuleForm.vue?raw')

    expect(router.default).not.toContain("path: 'egress-profiles'")
    expect(router.default).not.toContain("name: 'egress-profiles'")
    expect(sidebar.default).not.toContain('/egress-profiles')
    expect(bottomNav.default).not.toContain('/egress-profiles')
    expect(settingsPage.default).toContain('Egress Profiles')
    expect(settingsNav.default).toContain('v-for="tab in tabs"')
    expect(settingsPage.default).toContain("activeTab === 'egress'")
    expect(settingsPage.default).toContain('EgressProfilesPage')
    expect(page.default).toContain('useEgressProfiles')
    expect(page.default).toContain('EgressProfileForm')
    expect(page.default).toContain('deleteEgressProfile')
    expect(form.default).toContain('wireguard_config')
    expect(form.default).toContain('请重新输入代理 URL')
    expect(httpForm.default).toContain('useEgressProfiles')
    expect(httpForm.default).toContain('name="egress-profile"')
    expect(legacyHTTPForm.default).toContain('egress_profile_id')
    expect(l4Form.default).toContain('filteredEgressProfiles')
    expect(l4Form.default).toContain('profile.type !== \'http\'')
  })
})
