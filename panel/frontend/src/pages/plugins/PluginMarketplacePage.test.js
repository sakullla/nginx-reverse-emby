...
describe('PluginMarketplacePage', () => {
  ...
  it('opens an in-page inspect modal from the card instead of navigating to marketplace detail', async () => {
    const wrapper = mountPage()
    await flushPromises()
    expect(wrapper.find(`a[href="${detailHref()}"]`).exists()).toBe(false)
    await wrapper.get(`[data-test="marketplace-package-${entry.id}"]`).trigger('click')
    await flushPromises()
    expect(wrapper.find('.page-title').text()).toBe('插件市场')
    const inspect = wrapper.get('[data-test="marketplace-inspect-modal"]')
    expect(inspect.find('.modal-title').text()).toBe('WAF')
    expect(inspect.get('[data-test="marketplace-inspect-version"]').text()).toBe('1.2.0')
    expect(inspect.text()).toContain('未安装')
    expect(inspect.text()).toContain('非官方来源')
    expect(inspect.text()).toContain('安装后把插件部署到一个节点即可在该节点上使用。')
    expect(inspect.get('[data-test="marketplace-inspect-next"]').text()).toContain('下一步：部署到一个节点')
    expect(inspect.get('[data-test="marketplace-inspect-installed"]').text()).toBe('已安装插件')
    expect(inspect.get('[data-test="marketplace-inspect-installed"]').attributes('href')).toBe('/plugins')
    expect(inspect.get('[data-test="marketplace-inspect-installed"]').attributes('href')).not.toBe(`/plugins/${encodeURIComponent(entry.id)}`)
    expect(inspect.get('[data-test="marketplace-inspect-action"]').text()).toBe('安装')
    expect(mocks.push).not.toHaveBeenCalled()
    expect(wrapper.find(`a[href="${detailHref()}"]`).exists()).toBe(false)
    expect(mocks.fetchPluginPackageDetail).not.toHaveBeenCalled()
    await inspect.get('[data-test="marketplace-inspect-action"]').trigger('click')
    await flushPromises()
  ...
  it('keeps install, update and open on the card from opening the inspect modal', async () => {
    const wrapper = mountPage()
    await flushPromises()
    ...
    expect(wrapper.get('[data-test="marketplace-confirm-modal"]').find('.modal-title').text()).toBe('确认安装插件')
    expect(mocks.push).not.toHaveBeenCalled()
  })
  ...
  it('labels an installed package as open on the card and navigates to installed detail', async () => {
    ...
    await flushPromises()
    expect(wrapper.find('[data-test="marketplace-inspect-modal"]').exists()).toBe(false)
    expect(wrapper.find('[data-test="marketplace-confirm-modal"]').exists()).toBe(false)
    expect(mocks.installPlugin).not.toHaveBeenCalled()
    expect(mocks.push).toHaveBeenCalledWith(`/plugins/${encodeURIComponent(entry.id)}`)
    expect(mocks.push).not.toHaveBeenCalledWith(detailHref())
  })

  it('opens installed detail from inspect open, and sends 已安装插件 to the installed list', async () => {
    mocks.fetchPlugins.mockResolvedValue([{ plugin_id: entry.id, active_version: entry.version, active_package_digest: entry.sha256 }])
    const wrapper = mountPage()
    await flushPromises()
    await wrapper.get(`[data-test="marketplace-package-${entry.id}"]`).trigger('click')
    ...
    const inspect = wrapper.get('[data-test="marketplace-inspect-modal"]')
    expect(inspect.get('[data-test="marketplace-inspect-installed"]').attributes('href')).toBe('/plugins')
    expect(inspect.get('[data-test="marketplace-inspect-action"]').text()).toBe('打开')
    await inspect.get('[data-test="marketplace-inspect-action"]').trigger('click')
    await flushPromises()
    expect(mocks.installPlugin).not.toHaveBeenCalled()
    expect(mocks.push).toHaveBeenCalledWith(`/plugins/${encodeURIComponent(entry.id)}`)
    expect(mocks.push).not.toHaveBeenCalledWith('/plugins')
    expect(mocks.push).not.toHaveBeenCalledWith(detailHref())
  })
  ...
  it('switches the catalog to a list table with a visible install action and a name link', async () => {
    ...
    expect(table.find(`a[href="${detailHref()}"]`).exists()).toBe(false)
    expect(table.find(`[data-test="marketplace-detail-link-${entry.id}"]`).exists()).toBe(false)
