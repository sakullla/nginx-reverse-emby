...
<style scoped>
.result-summary { display: grid; grid-template-columns: repeat(2, 1fr); gap: var(--space-3); }
.result-card {
...
.result-card__label { font-size: var(--text-sm); color: var(--color-text-secondary); }
.result-card__value { font-size: var(--text-2xl); font-weight: var(--font-bold); color: var(--color-text-primary); }
.result-card__value--ok { color: var(--color-success); }
...
.info-row { display: flex; align-items: center; justify-content: space-between; padding: var(--space-2) 0; border-bottom: 1px solid var(--color-border-subtle); }
...
.report-details__summary::before { content: '▸'; margin-right: var(--space-2); display: inline-block; transition: transform var(--duration-fast); }
.report-details[open] .report-details__summary::before { transform: rotate(90deg); }
.report-group { padding: 0 var(--space-4) var(--space-4); }
.report-block + .report-block { border-top: 1px solid var(--color-border-subtle); }
.report-block__header { display: flex; justify-content: space-between; gap: var(--space-4); padding: var(--space-3) var(--space-4); font-size: var(--text-sm); font-weight: var(--font-semibold); }
.report-block__count { color: var(--color-text-secondary); }
.report-list { list-style: none; margin: 0; padding: 0 var(--space-4) var(--space-4); display: flex; flex-direction: column; gap: var(--space-2-5); }
.report-list__item { display: grid; grid-template-columns: 120px 1fr; gap: var(--space-2) var(--space-3); align-items: start; }
.report-list__kind { font-size: var(--text-xs); color: var(--color-text-secondary); text-transform: uppercase; letter-spacing: 0.04em; }
.report-list__key { color: var(--color-text-primary); word-break: break-all; }
.report-list__reason { grid-column: 2; font-size: var(--text-sm); color: var(--color-text-tertiary); }
.report-empty { padding: 0 var(--space-4) var(--space-4); font-size: var(--text-sm); color: var(--color-text-tertiary); }

@media (max-width: 480px) { .result-summary { grid-template-columns: 1fr; } }
</style>
