...
const props = defineProps({ component: { type: Object, required: true }, model: { type: Object, required: true }, secretReplacements: { type: Object, default: () => ({}) }, secretPresent: { type: Function, default: () => false } })
...
<template>
  <fieldset v-if="component.type === 'section'" class="declarative-section">
    ...
    <PluginDeclarativeComponent v-for="child in component.children || []" :key="child.id" :component="child" :model="model" :secret-replacements="secretReplacements" :secret-present="secretPresent" @change="(...args) => emit('change', ...args)" @secret="(...args) => emit('secret', ...args)" />
  </fieldset>
  ...
  <label v-else-if="component.type === 'secret'" class="declarative-field declarative-secret"><span>{{ component.label }}</span><input type="password" autocomplete="new-password" :value="secretReplacements[component.binding] ?? ''" :required="component.required && !secretPresent(component.binding)" :readonly="component.read_only" placeholder="留空以保留现有凭据" @input="emit('secret', component.binding, $event.target.value)"><small>{{ secretPresent(component.binding) ? '已有凭据；留空保留，输入新值轮换。' : '尚未配置；明文仅随本次提交发送。' }}</small><button v-if="secretPresent(component.binding) && !component.required && !component.read_only" class="btn btn-secondary" type="button" @click="emit('secret', component.binding, null)">清除凭据</button></label>
