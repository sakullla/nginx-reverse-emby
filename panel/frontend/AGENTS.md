...
This repository combines a Vue 3 control panel with a lightweight Node backend and container tooling. The SPA lives in `panel/frontend/`: application code is in `src/`, reusable UI in `src/components/`, API clients in `src/api/`, state stores in `src/stores/`, and shared styles in `src/styles/`. The backend entrypoint is `panel/backend/server.js`. Container startup scripts and Nginx templates live in `docker/`, while helper scripts such as `join-agent.sh` are in `scripts/`. Treat `panel/data/` as runtime data only; never commit generated files, certificates, or secrets.
...
- `docker exec nginx-reverse-emby nginx -t` — validate generated Nginx config after proxy or template changes.

## Coding Style & Naming Conventions
Use 2-space indentation throughout the frontend. Vue components should use `<script setup>`, single quotes, and no semicolons. Backend code follows CommonJS (`require`) with semicolons and defensive error handling. Keep shell scripts in `docker/` POSIX-friendly; `scripts/join-agent.sh` may use Bash features. Name Vue components with `PascalCase.vue` and JavaScript modules/stores in `camelCase`. Environment variables should remain uppercase with prefixes such as `PANEL_`, `PROXY_`, `ACME_`, `MASTER_`, and `AGENT_`.
...
For Nginx, certificate, proxy, or L4 changes, also validate with `nginx -t` inside a running container.
