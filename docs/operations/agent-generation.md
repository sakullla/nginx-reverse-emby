...
普通修改和新增会优雅排空旧会话；delete 或 `enabled=false` 在新 generation ready 后立即关闭受影响旧会话。HTTP/1.1、HTTP/2、HTTP/3、WebSocket、L4 TCP/UDP、Relay 和 WireGuard 都遵守 generation ownership；UDP 按旧五元组/association 继续旧路径，新五元组进入 active generation。
...
Timeout 在 revision 创建时按“agent 字段覆盖 -> global”解析并以秒固化。之后修改环境变量只影响新 revision，不会改变正在 apply/draining 的 generation。同一 agent/module 最多保留 2 个 draining generation；创建第三个时强制释放最老一代并记录 `generation_limit`。
