...
非 Linux、其它 architecture 或未声明对应 package/capability 的 agent 会在 preflight 显式拒绝，不会降级为有停机的进程替换。
...
2. Child ready 后新连接进入目标 generation；旧 TCP 会话与 UDP/QUIC/WireGuard association 仍由原 owner 服务。
...
Child prepare、activation、authority transfer 或 identity 校验失败时，parent 保持/恢复 authority 和 last-known-good generation，失败 child 被终止，current pointer 恢复 previous。失败不会把未 ready generation 报为 applied。
...
- `unsupported`：核对 agent 报告的平台是否为 Linux amd64/arm64，且 version policy 有精确 platform package；不要强制使用其它平台 binary。
