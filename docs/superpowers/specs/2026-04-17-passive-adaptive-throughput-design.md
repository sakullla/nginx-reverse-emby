...
2. HTTP 具有清晰的响应体传输语义，而 TCP 长连接/流式连接缺少稳定的响应边界，若直接在 copy 热路径上做吞吐学习，容易把客户端读速、长连接空闲和下游抖动误判为后端能力差异。
...
5. 让 HTTP 运行时和 HTTP 诊断页使用同一套吞吐口径，同时保持 L4/TCP 继续按延迟与稳定性排序。

## 非目标

1. 本轮不引入主动测速、后台定时探测或额外探针请求。
2. 本轮不修改控制面 API 结构，不新增控制面配置项。
3. 本轮不将 TCP 或 UDP 纳入持续吞吐排序模型，L4 仍只学习延迟与可用性。
4. 本轮不在 TCP `io.Copy` 热路径中增加持续吞吐观测。
5. 本轮不处理 JSON 字段名兼容迁移，`estimated_bandwidth_bps` 暂时保留。
...
- TCP/UDP: 本轮不定义 `transferDuration`，不参与持续吞吐模型

## 样本分层

所有成功 HTTP 请求都继续学习延迟；只有满足条件的真实业务 HTTP 响应才学习吞吐。
...
建议新增一组按小时滚动的 HTTP 流量结构桶，与现有成功/失败计数桶保持相同的 `24h` 观测窗口。每个小时桶至少记录：
...
- HTTP 自适应排序使用本设计中的动态双指标模型。
- TCP/L4 自适应排序不使用吞吐，`performance` 继续按延迟信号计算。
- 任何由 HTTP 真实流量学习到的吞吐信息都不能影响 TCP/L4 的候选排序，即使它们恰好共享同一个解析后的 `ip:port`。
...
TCP/L4 本轮不引入持续吞吐观测，继续维持当前保守语义：

1. 连接成功时继续学习建连延迟
2. 继续学习稳定性、失败回退、恢复慢启动与冷启动探索
3. 不在 `io.Copy` 热路径中记录吞吐样本
4. 不让地址级 HTTP 吞吐观测泄漏到 TCP/L4 排序

原因是 TCP 缺少稳定的应用层响应边界，若直接在 copy 路径上度量吞吐，容易把客户端读速、长连接空闲或下游网络问题错误归因到后端能力。

### TCP/L4 诊断

TCP/L4 诊断本轮也不再提供任何“带宽”或“持续吞吐”评估：

1. L4 诊断摘要与子节点摘要不输出吞吐字段
2. L4 诊断 UI 不展示吞吐相关指标
3. L4 诊断继续只展示延迟、成功率、稳定性、恢复状态和流量分配提示

这样可以避免同一产品里 HTTP 有吞吐语义、TCP 却展示一个不可靠的“伪带宽”指标。
...
7. TCP/L4 继续不学习吞吐，排序保持稳定性与延迟优先
8. HTTP 地址级吞吐观测不会泄漏到 TCP/L4 排序
9. TCP/L4 诊断不再输出或展示带宽/持续吞吐评估
10. UDP 保持当前仅学习延迟与可用性的行为
11. HTTP 诊断页摘要与运行时摘要对吞吐的可见性和数值口径一致
...
2. 由于 TCP/L4 本轮不参与持续吞吐，L4 在大对象场景下仍然只能依靠延迟与稳定性做排序。
   这是有意取舍，优先避免将客户端读速或流式空闲误判成后端吞吐能力。
