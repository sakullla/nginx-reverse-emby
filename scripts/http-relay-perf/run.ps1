...
$delayCliToHttp = Get-OptionalEnvInt 'HARNESS_DELAY_CLI_TO_HTTP_MS'
$delayHttpToBackend = Get-OptionalEnvInt 'HARNESS_DELAY_HTTP_TO_BACKEND_MS'
$fallbackDelay = Get-OptionalEnvInt 'HARNESS_NETEM_DELAY_MS'
if ($null -eq $delayCliToHttp) {
    $delayCliToHttp = $defaultDelayCliToHttp
}
if ($null -eq $delayHttpToBackend -and $null -ne $fallbackDelay) {
    $delayHttpToBackend = $fallbackDelay
}
if ($null -eq $delayHttpToBackend) {
    $delayHttpToBackend = $defaultDelayHttpToBackend
}

if (
    ($delayCliToHttp -gt 0 -or $delayHttpToBackend -gt 0) -and
...
if ($delayCliToHttp -gt 0) {
    Apply-NetemDelayOnCIDR -Container 'nre-perf' -CIDR '172.31.1.2/24' -DelayMs $delayCliToHttp
    Apply-NetemDelayOnCIDR -Container 'nre-agent-a' -CIDR '172.31.1.10/24' -DelayMs $delayCliToHttp
}
