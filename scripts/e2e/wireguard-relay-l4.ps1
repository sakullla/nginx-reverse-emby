...
$ClientConfigPath = Join-Path $PSScriptRoot 'wireguard-relay-l4-client.conf'
$AgentIDs = @{}
...
function Start-ControlPlane {
  ...
  Wait-HTTP -Uri "http://127.0.0.1:8080/" -Retries 1 *> $null
}
...
with Server(('0.0.0.0', 18080), Handler) as srv:
    srv.serve_forever()
PY" | Out-Null
}
...
function Get-WireGuardProfilePrimaryHost {
  param([object]$Profile)

  if ($null -eq $Profile -or $null -eq $Profile.addresses) {
    throw 'WireGuard profile is missing addresses'
  }
  $address = @($Profile.addresses | Where-Object { -not [string]::IsNullOrWhiteSpace($_) } | Select-Object -First 1)
  if ($address.Count -eq 0) {
    throw 'WireGuard profile has no usable address'
  }
  $cidr = [string]$address[0]
  $slash = $cidr.IndexOf('/')
  if ($slash -lt 1) {
    throw "WireGuard profile address is not CIDR: $cidr"
  }
  return $cidr.Substring(0, $slash)
}
...
while [ "$i" -le 30 ]; do
  if wget -q -O - --timeout=3 http://TARGET_HOST_PLACEHOLDER:18080/ | grep -q 'wg-e2e-ok'; then
    echo PASS tcp wg-client '->' A transparent '->' RelayA tls_tcp '->' RelayB wireguard '->' B
    exit 0
  fi
  i=$((i + 1))
  sleep 1
...
try {
  ...
  foreach ($agentName in @('a', 'relay-a', 'relay-b', 'b')) {
  ...
  Write-Section "Creating mixed relay listeners"
  ...
  $relayB = New-RelayListener -AgentID $AgentIDs['relay-b'] -Body @{
    name                = 'relay-b-wireguard'
    listen_port         = 19002
    public_host         = (Get-ContainerName 'relay-b')
    public_port         = 51820
  ...
  Write-Section "Creating TCP-only WireGuard transparent L4 rule"
  New-L4Rule -AgentID $AgentIDs['a'] -Body @{
    ...
    listen_host             = '0.0.0.0'
    listen_port             = 18080
    relay_layers            = @(@($relayA.id), @($relayB.id))
  ...
  Write-Host 'NOTE UDP transparent is intentionally not exercised here because current support is deferred.' -ForegroundColor Yellow

  Write-Section "Waiting for apply status"
  Wait-AgentStatus -AgentIDs @($AgentIDs['a'], $AgentIDs['relay-a'], $AgentIDs['relay-b'], $AgentIDs['b'])

  Write-Section "Fetching generated client config"
  $configPath = New-WireGuardClientConfig -AgentID $AgentIDs['a']

  Write-Section "Running real WireGuard client probe"
  $targetHost = Get-ContainerIPAddress -Name (Get-ContainerName 'backend')
