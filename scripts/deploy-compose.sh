...
install_docker_compose() {
    ...
    if ! ask_yes_no "是否现在自动安装 Docker Compose" "y"; then
        echo "请先安装 Docker 与 Compose 插件后重新运行本脚本。" >&2
        exit 1
    fi
    ...
    if command -v apt-get >/dev/null 2>&1; then
        ...
        echo "暂不支持自动安装当前系统的软件包管理器。请手动安装 Docker Engine 和 Docker Compose 插件。" >&2
        exit 1
    fi
...
if [ -n "$public_url" ]; then
    ...
    echo "建议权限：Zone:Read + DNS:Edit；Zone Resources 只选择你的域名，不要使用 Global API Key。" >&2
