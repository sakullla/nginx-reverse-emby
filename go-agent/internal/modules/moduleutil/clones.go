...
func CloneHTTPRules(rules []model.HTTPRule) []model.HTTPRule {
	...
	for i, rule := range rules {
		...
		cloneRuleSharedFields(&cloned[i].RelayChain, &cloned[i].RelayLayers, &cloned[i].Tags, &cloned[i].EgressProfileID, rule.RelayChain, rule.RelayLayers, rule.Tags, rule.EgressProfileID)
	}
	return cloned
}
...
func cloneRuleSharedFields(relayChain *[]int, relayLayers *[][]int, tags *[]string, egressProfileID **int, sourceRelayChain []int, sourceRelayLayers [][]int, sourceTags []string, sourceEgressProfileID *int) {
