...
func TestIntegrationLegacyExecutionScopeChild(t *testing.T) {
	...
	if _, present := os.LookupEnv(sdk.EnvPluginExecutionScope); present || os.Getenv(sdk.EnvPluginHostEndpoint) != "" {
		t.Fatal("legacy package received a new execution protocol")
	}
	// This is the original pre-scope SDK face rule, without the new helper.
	if strings.TrimSpace(os.Getenv(sdk.EnvPluginHostEndpoint)) != "" {
		t.Fatal("legacy guest selected control-plane")
	}
	adapter, err := rpcplugin.NewAdapter(rpcplugin.Config{PluginID: "legacy.scope", PluginVersion: "1.0.0", RequiredGrants: []string{"agent.read"}, Timeouts: rpcplugin.UniformTimeouts(5 * time.Second)}, rpcplugin.HookFuncs{})
	if err != nil {
		t.Fatal(err)
	}
...
func TestIntegrationLegacyGuestScopeCompatibilityAndOverrideRefusal(t *testing.T) {
	...
	requirement, err := pluginprocess.NewSandboxRequirement(pluginprocess.SandboxRequirementProjection{PackageDigest: digest, Permissions: []pluginprocess.SandboxPermission{"agent.read"}, ResourceBudget: pluginprocess.ManifestResourceBudget{TimeoutMS: 5000, MemoryBytes: 256 << 20, Concurrency: 4, InputBytes: 4096, OutputBytes: 4096, CPUMillis: 1000}})
	if err != nil {
		t.Fatal(err)
	}
