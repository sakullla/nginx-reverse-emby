...
func TestRevisionSyncSuccessfulNoUpdateRepairsMissingEntrypointApplyError(t *testing.T) {
	events := []string{}
	store := newRevisionTestStore(&events)
	applied := revisionSnapshot(7)
	runtime := NewRuntime()
	...
	packageMessage := "open current executable: open fixed-entrypoint: no such file or directory"
	if err := store.SaveRuntimeState(RuntimeState{
		CurrentRevision: applied.Revision,
		Metadata: map[string]string{
			"last_sync_error":     packageMessage,
	...
	if state.Metadata["last_apply_revision"] != "7" || state.Metadata["last_apply_status"] != "success" || state.Metadata["last_apply_message"] != "" {
		t.Fatalf("missing entrypoint apply metadata = %+v, want repaired success", state.Metadata)
	}
}

func TestRevisionSyncSuccessfulNoUpdateRepairsLegacyRestartRequestApplyError(t *testing.T) {
	testCases := map[string]string{
		"sentinel": ErrRestartRequested.Error(),
		"wrapped":  fmt.Errorf("update handoff: %w", ErrRestartRequested).Error(),
		"joined acknowledgement replay": errors.Join(
			errors.New("acknowledgement replay failed"),
			ErrRestartRequested,
		).Error(),
	}
	for name, restartMessage := range testCases {
...
func revisionSnapshot(revision int64) model.Snapshot {
	payload := fmt.Sprintf(`{"desired_version":"v%d","desired_revision":%d,"agent_config":{},"rules":[],"l4_rules":[],"relay_listeners":[],"egress_profiles":[],"certificates":[],"certificate_policies":[],"plugin_policies":[]}`, revision, revision)
