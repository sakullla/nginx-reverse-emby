...
type RuntimeNet interface {
	...
	ListenUDP(addr *net.UDPAddr) (*gonet.UDPConn, error)
...
func (net *Net) ListenTCP(addr *net.TCPAddr) (net.Listener, error) {
	if addr == nil {
...
func (tnet *Net) DialContext(ctx context.Context, network, address string) (net.Conn, error) {
	...
	for _, addr := range allAddr {
		...
		if parseErr == nil && ((ip.Is4() && acceptV4) || (ip.Is6() && acceptV6)) {
	...
	for i, addr := range addrs {
		...
		if deadline, hasDeadline := ctx.Deadline(); hasDeadline {
			partial, partialErr := partialDeadline(time.Now(), deadline, len(addrs)-i)
			if partialErr != nil {
				if firstErr == nil {
					firstErr = &net.OpError{Op: "dial", Err: partialErr}
				}
				break
			}
			if partial.Before(deadline) {
				var cancel context.CancelFunc
				dialCtx, cancel = context.WithDeadline(ctx, partial)
				defer cancel()
			}
		}
