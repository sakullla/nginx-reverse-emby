...
func TestFilesystemRejectsInvalidRelativePaths(t *testing.T) {
	t.Parallel()

	for _, name := range []string{"", ".", "../outside", "/absolute", "accounts/../outside", "accounts//outside", `accounts\..\outside`} {
		if err := validateRelativePath(name, false); err == nil {
			t.Errorf("validateRelativePath(%q, false) error = nil", name)
		}
	}
	if err := validateRelativePath(".", true); err != nil {
		t.Fatalf("validateRelativePath(root) error = %v", err)
	}
	if err := validateRelativePath("accounts/0123456789abcdef", false); err != nil {
